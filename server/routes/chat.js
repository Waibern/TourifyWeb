...
function allowed(ip) {
  const now = Date.now();
  const recent = (visitors.get(ip) || []).filter(
...
function isIndonesian(text) {
  return /\b(halo|hai|tolong|wisata|tiket|destinasi|harga|keluarga|liburan|berapa|yang|untuk|dimana|di mana)\b/i.test(
    text,
  );
}
...
function fallbackReply(text, destinations, indonesian) {
  ...
  const matches = destinations
    .filter((item) => {
      ...
      return (
        (family &&
          /family|anak|all ages|water park|theme park/.test(haystack)) ||
        (water && /water/.test(haystack)) ||
        (nature && /nature|wildlife|safari/.test(haystack))
      );
    })
    .slice(0, 3);
...
router.post("/", async (req, res, next) => {
  try {
    ...
    let reply = null;
    try {
      reply = await askProvider(messages, destinations);
    } catch (error) {
      console.warn("AI provider unavailable:", error.message);
    }
    res.json({
      reply: reply || fallbackReply(latest, destinations, isIndonesian(latest)),
